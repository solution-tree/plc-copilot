...
editHistory:
  - date: '2026-02-25'
    changes: 'Initial v4.1 draft'
  - date: '2026-02-26'
    changes: 'Merged v4.1 and v4.2: added enumerated FRs (FR-1–FR-14), expanded NFRs to 19, added ambiguous query category to golden dataset, Flow D API example, tunable retrieval parameters, ACs 14–15, Decision Log entries 11–13; applied measurability fixes to AC-12/13 and implementation leakage cleanup throughout'
---
...
| NFR ID | Category | Requirement | Measurement Method |
|---|---|---|---|
| NFR-11 | Encryption | TLS 1.2+ enforced on all connections (load balancer, RDS, ElastiCache, OpenAI) | Automated certificate/protocol checks in CI/CD |
...
| NFR-16 | Data Retention | Query audit logs retained for 90 days. Redis session data per NFR-15. Relational book/chunk data retained indefinitely. | Infrastructure configuration review |
...
The API service shall auto-generate OpenAPI 3.0 documentation, accessible at the standard `/docs` endpoint. This serves as the live API reference during internal testing.
...
5. The ingestion pipeline successfully processes all 25 source PDFs from the source corpus bucket into the vector store and the metadata database.
